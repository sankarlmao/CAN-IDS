...
## Executive Summary
In this session, the CAN Intrusion Detection System (CAN-IDS) was upgraded from a basic LED indicator into a full-scale Automotive CAN Intrusion Prevention System (IPS) with active hardware failsafe actuators, real-time Wokwi microcontroller integration, and a Web Cyber Security Operations Center (CSOC) Dashboard.

---

## Key Changes and System Enhancements

### 1. Hardware Actuators and Active Defense Integration (`stm32_wokwi/sketch.ino` & `diagram.json`)
- **SG90 Servo Motor Actuator (`D6`)**: Added as a physical Vehicle Safe-Stop Emergency Brake Actuator. Rotates from 90 degrees (Normal Drive) to 0 degrees (Emergency Safe-Stop) when TinyML anomaly score exceeds threshold (>0.30).
...
- **Serial Remote Command Listener and JSON Telemetry**:
...
- Replaced basic LED with:
  ...
  - 4 Pushbuttons (`D2`, `D3`, `D4`, `D5`) for hardware traffic injection.

### 3. Web CSOC Dashboard and Telemetry Visualizer (`web_dashboard/`)
...
- **`styles.css`**: Automotive dark theme with glassmorphism, status indicators, and responsive grid system.
...
### Launch Web CSOC Visualizer and Server
```bash
python3 stm32_wokwi/wokwi_bridge.py --port 8080
```
*Open `http://localhost:8080` in your browser.*

### Build and Run Local Host C++ Simulator
```bash
cd stm32_wokwi
make clean && make
./run_sim
