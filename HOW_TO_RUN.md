# 🚀 Complete Execution & Setup Guide: CAN Sentinel IPS

This document provides a comprehensive, step-by-step walkthrough explaining how to run and demonstrate every layer of the **CAN Sentinel Automotive Intrusion Prevention System (IPS)**.
...
- **Web Browser**: Google Chrome, Microsoft Edge, Brave, or Opera (Recommended for Web Serial API support)
...
## 🚘 Method 1: Running the Interactive Web CSOC Dashboard (Recommended for Demos)

The Web Cyber Security Operations Center (CSOC) Dashboard provides a real-time, interactive visualization of the vehicle network topology, active hardware failsafe actuators, OLED screen mirror, waveform charts, and acoustic sirens.
...
### Step 3: How to Interact with the Visualizer
1. **Traffic Stream Selectors**: Click the 4 scenario buttons on the left panel:
   - 🟢 **Normal Traffic**: Simulates clean telemetry. Anomaly score remains low (`~0.02`), vehicle throttle is open (`90°`), gateway relay is connected (`CLOSED`).
   - 🔴 **DoS Attack**: Simulates a high-rate `0x000` arbitration ID packet flood. Anomaly score spikes (`>0.79`), triggering emergency safe-stop (`0°`), relay trip (`OPEN`), and acoustic sirens.
...
## 🎛️ Method 2: Running the Microcontroller Circuit Simulation in Wokwi
...
|---|---|
| `sketch.ino` | Copy contents from [`stm32_wokwi/sketch.ino`](stm32_wokwi/sketch.ino) |
| `diagram.json` | Replace Wokwi `diagram.json` with contents from [`stm32_wokwi/diagram.json`](stm32_wokwi/diagram.json) |
| `test_buffers.h` | Create new file `test_buffers.h` and paste from [`stm32_wokwi/test_buffers.h`](stm32_wokwi/test_buffers.h) |
| `ei_run_classifier.h` | Create new file `ei_run_classifier.h` and paste from [`stm32_wokwi/edge-impulse-sdk/classifier/ei_run_classifier.h`](stm32_wokwi/edge-impulse-sdk/classifier/ei_run_classifier.h) |
...
2. Press the hardware colored push-buttons in Wokwi:
...
3. Watch the physical components react in real time:
   - **SG90 Servo**: Rotates from 90° to 0° on attack.
